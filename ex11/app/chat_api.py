"""채팅 API 라우터 — ex11 완성 에이전트 + RAG + PDF 캡처 이미지 근거.

CH11 확장:
- `_classify_query_category`: 질문을 정형/비정형 키워드 규칙으로 분류
- `_attach_evidence`: 비정형 질문이면 top-1 source+page를 PDF에서 PNG로 캡처해 첨부
- ChatResponse에 `category`(정형/비정형), `evidence_json`(정형 근거) 필드 추가
"""
...
class ChatResponse(BaseModel):
    ...
    category: str = Field(default="unstructured", description="근거 카테고리: structured(정형) | unstructured(비정형)")
    ...
    evidence_json: dict = Field(default_factory=dict, description="정형 질문의 JSON 근거(숫자·수치)")


# ---------------------------------------------------------------------------
# 2a. 카테고리 분류 + 근거 첨부 헬퍼 (CH11 신규)
# ---------------------------------------------------------------------------

# 숫자·금액·수치·날짜 키워드 → 정형 질문
...
def _classify_query_category(query: str) -> str:
    """질문을 정형(structured) / 비정형(unstructured)로 분류한다.

    키워드 규칙 v1:
    - 질문에 숫자 토큰이 있거나 _STRUCTURED_KEYWORDS 중 하나라도 포함 → structured
    - 그 외 → unstructured
    LLM 분류가 필요한 실무에서는 이 함수를 LLM 호출로 대체하면 된다.
    """
...
def _attach_evidence_images(docs: list) -> list:
    """검색된 문서 중 top-1부터 source+page로 PDF 페이지를 PNG로 렌더링한다.

    Returns:
        list[dict]: [{"url": "/static/evidence/...", "source": "...", "page": 1}, ...]
    """
    src_path = os.path.join(_BASE_DIR, "src")
    if src_path not in sys.path:
        sys.path.insert(0, src_path)

    from src.evidence_capture import render_evidence_image

    images = []
    for doc in docs[:3]:  # 상위 3개까지만
        ...
        try:
            ...
            page_int = 1
        url = render_evidence_image(source, page_int)
        if url:
            images.append({"url": url, "source": source, "page": page_int})
    return images
...
async def chat_endpoint(body: ChatRequest):
    ...
    if body.use_agent:
        ...
        try:
            ...
            # CH11: 카테고리 분류 + 근거 모양 분기
            category = _classify_query_category(body.query)
            evidence_json: dict = {}
            if category == "structured":
                # 정형 질문: DB 결과를 그대로 JSON으로 전달
                evidence_json = structured_data
            elif not evidence_images and unstructured_data:
                # 비정형 질문: evidence_images가 없으면 top 문서 PDF 페이지를 즉석 캡처
                evidence_images = _attach_evidence_images(unstructured_data)
        ...
        try:
            ...
            # CH11: RAG 모드도 카테고리 분류 + 비정형이면 근거 이미지 자동 생성
            category = _classify_query_category(body.query)
            if category == "unstructured" and not evidence_images and docs:
                evidence_images = _attach_evidence_images(docs)
