...
function appendAiMessage(data) {
  ...
  // JSON 근거 (정형)
  const jsonSection = buildEvidenceJson(data);
...
/**
 * 정형 질문의 JSON 근거 섹션을 생성한다 (CH11).
 * - data.evidence_json: DB 조회 결과 딕셔너리
 */
function buildEvidenceJson(data) {
  const ejson = data.evidence_json || {};
  if (!ejson || Object.keys(ejson).length === 0) return '';

  const pretty = escapeHtml(JSON.stringify(ejson, null, 2));
  return `
    <div class="evidence-json">
      <div class="evidence-json-title">근거 JSON (정형)</div>
      <pre class="evidence-json-block"><code>${pretty}</code></pre>
    </div>
  `;
}
